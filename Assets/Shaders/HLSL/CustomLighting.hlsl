...
float GetDiffuse(CustomLightingData d) {
    #ifdef SHADERGRAPH_PREVIEW
        return 1;
    #else
        Light light = GetMainLight(d.shadowCoord, d.positionWS, 1);
        return saturate(dot(d.normalWS, light.direction));
    #endif
}

void CalculateCustomLighting_float(float3 Position, float3 Normal, float3 ViewDirection, float3 Albedo, float Smoothness, out float3 Color, out float Diffuse) {
